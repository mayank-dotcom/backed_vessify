...
import { authMiddleware, getCurrentOrganization, AuthContext } from '../middleware/auth'
...
transactionRoutes.post('/extract', async (c) => {
    try {
        ...
        const parseResult = parseTransaction(text)
        ...
        const organizationId = await getCurrentOrganization(c)
